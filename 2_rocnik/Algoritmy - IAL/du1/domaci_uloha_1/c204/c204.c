...
void untilLeftPar( Stack *stack, char *postfixExpression, unsigned *postfixExpressionLength ) {
	...
	// Cyklus postupně projíždí stack a vkládá jeho hodnoty do řetezce, dokud se na vrcholu stacku není '('
	while(top_char != '(' && !Stack_IsEmpty(stack)){
...
void doOperation( Stack *stack, char c, char *postfixExpression, unsigned *postfixExpressionLength ) {
	...
	// Pokud stack není prázdyn hodnota vrchního indexu se uloží do proměnné
	if(!Stack_IsEmpty(stack)){
		Stack_Top(stack, &top_char);
	}
		
	// Určí se priorita daného operátoru, podle které se pak operátor vloži na stack, nebo se rovnou vloží na konec řetězce
	if(Stack_IsEmpty(stack) || top_char == '(' || (((top_char == '+' && c != '-' && c != '+') || (top_char == '-' && c != '-' && c != '+') ))) {
	...
	// Funkce využívá rekurze,aby bylo zajištěno správné pořadí operátorů podle jejich priorit
...
char *infix2postfix( const char *infixExpression ) {
	...
	// Alokace paměti pro výsledný řetězec
	postfixExpression = (char *) malloc(MAX_LEN*sizeof(char));
	
	// Alokace paměti pro stack
	Stack *stack =  (Stack *) malloc(sizeof(Stack));

	// Kontrola správného průběhu alokování
	...
	// Cyslkus postupně projíždí infixní formu řetězce dokud nedojde nakonec
	for(char c = *infixExpression; c != '\0'; c = *(++infixExpression)){

		// Pokud se jedná o operátor volá sse funkce pro rozpoznání priority
		if(c == '+' || c == '-' || c == '*' || c == '/'){
		...
		// Pokud se jedná o proměnou, tak je vložena na konec řetezce
		...
		// Na konci řetězce je vše postupně přidáno do výsledného řetězce
		else if (c == '=') {
...
void expr_value_push( Stack *stack, int value ) {
	...
	// Pomocí maskování je int rozdělen na jednotlivé byty
	char bytes[4];
...
bool eval( const char *infixExpression, VariableValue variableValues[], int variableValueCount, int *value ) {
	...
	Stack *stack_eval = (Stack *) malloc(sizeof(Stack));
	Stack_Init(stack_eval);
	
	char *postfixEx = infix2postfix(infixExpression);
	
	if(stack_eval == NULL || postfixEx == NULL){
		return NULL;
	}
	
	for(int i = 0; postfixEx[i] != '='; i++){

		char current_char = postfixEx[i];
		...
		if(current_char == '+' || current_char == '-' || current_char == '*' || current_char == '/'){
			...
			expr_value_pop(stack_eval, &num2);
			
			expr_value_pop(stack_eval, &num1);
			...
			expr_value_push(stack_eval, result);
			...
			for(int j = 0; j < variableValueCount; j++){
           		if(variableValues[j].name == current_char){
					...
					int num = variableValues[j].value; 
					
                	expr_value_push(stack_eval, num);
	...
	expr_value_pop(stack_eval, value);
	Stack_Dispose(stack_eval);
	free(stack_eval);
	free(postfixEx);
