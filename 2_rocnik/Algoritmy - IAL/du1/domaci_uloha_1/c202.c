...
void Stack_Init( Stack *stack ) {
	...
	// Alokace paměti a inicializace na prázdný stack
...
void Stack_Pop( Stack *stack ) {
	...
	// Pokud stack není prázdný, vymaže se hodnota první položky a posune se index vrchní položky
	if(!Stack_IsEmpty(stack)){
...
void Stack_Push( Stack *stack, char data ) {
	...
	// Pokud stack není plný, index vrchní položky se posune a uloží do něj hodnota
	stack->topIndex++;
...
void Stack_Dispose( Stack *stack ) {
	...
	// Uvede stack do prázdného stavu a odalokuje naalokovanou paměť
	stack->topIndex = -1;
	stack->array = NULL;
	free(stack->array);
