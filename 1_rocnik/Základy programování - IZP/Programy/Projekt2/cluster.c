...
int load_clusters(char *filename, struct cluster_t **arr)
{
    ...
    {
        char exten[] = ".txt";
        char filename_exten[25];
        sprintf(filename_exten,"%s%s",filename,exten);
        file = fopen(filename_exten, "r");
        if(file == NULL)
        {
            return -1;
        }
    ...
    struct cluster_t *c = (struct cluster_t *)malloc(sizeof(struct cluster_t) * count);
   
    for (int i = 0; i < count; i++)
    {
        init_cluster(&c[i], 1);
        fscanf(file,"%d %f %f", &c[i].obj->id, &c[i].obj->x, &c[i].obj->y);
        c[i].size = 1;
...
int main(int argc, char *argv[])
{
    ...
    if(argc == 3)
    {
        ...
        if(cluster_count == -1)
        {
            printf("CHYBA: Spatne zadany soubor");
            return 1;
        }
        final_count = atoi(argv[2]);
        
        if(final_count < 2 || final_count > cluster_count)
        {
            printf("CHYBA: Zadany spatny pocet hledanych shluku");
            return 1;
        }
     
      
        for(int i = cluster_count; i > final_count ; i--)
        {
    ...
    {
    printf("CHYBA: Zadan spatny pocet argumentu");
    return 1;
    }
